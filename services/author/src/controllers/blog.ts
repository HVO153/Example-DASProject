...
import { uploadBlogToS3 } from "../utils/s3Uploader.js";
...
export const createBlog = TryCatch(async (req: AuthenticatedRequest, res) => {
    ...
    await uploadBlogToS3(result[0].id, result[0]);
...
export const aiTitleResponse = TryCatch(async (req, res) => {
    ...
    const prompt = `Correct the grammar of the following blog title and 
    return only the corrected title without any additional text, 
    formatting, or symbols: "${text}"`;
